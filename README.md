...
Output files are "_new.asc" files.

2.- mergeHeartEyeEEG() <br />

This function combines eye movement and EEG/ECG data using the "EYE-EEG" plugin based on common events, downsample the EEG data to 250 Hz, and merges EEG data from the linguistic and nonlinguistic recordings. <br />

Input files are (1) eye movement files: "_new.asc" files (each participant has 2 under the "Eye movement" folder: one for linguistic, one for nonlinguistic) generated by the "replaceEyeLinkStrings.m" function above; (2) EEG files: ".eeg", ".vhdr", ".vmrk" files (each participant has 6 under the "Eye movement" folder: 3 for linguistic, 3 for nonlinguistic). <br />
Output files are ".set" and ".fdt" files (in EEGLAB format), named by the participant number.
